...
export async function entertainDialog(config: any, hass: any): Promise<void> {
    ...
    const videoPlayerObj = config.video_player
        ? hass.states[config.video_player]
        : undefined;
    const videoSoundObj = config.video_sound
        ? hass.states[config.video_sound]
        : undefined;
    const audioPlayerObj = config.audio_player
